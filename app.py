...
# Clase Canvas personalizada para dibujar Encabezado y Pie de página
class PlantillaOficialYucatan(canvas.Canvas):
    ...
    def dibujar_membrete(self):
        ...
        if os.path.exists("encabezado.png"):
            self.drawImage("encabezado.png", 36, alto - 85, width=ancho - 72, height=65, preserveAspectRatio=True, mask='auto')
        
        # --- PIE DE PÁGINA ---
        if os.path.exists("pie_pagina.png"):
            self.drawImage("pie_pagina.png", 36, 20, width=ancho - 72, height=45, preserveAspectRatio=True, mask='auto')

# Función que genera el documento PDF oficial
def generar_pdf_oficial(contenido_relatoria):
    buffer = io.BytesIO()
    
    # Márgenes ajustados para no encimar el texto con las imágenes
...
if audio_path_temporal:
    ...
    if st.button("🚀 Procesar Audio y Generar Relatoría Oficial", type="primary", use_container_width=True):
        ...
        Redacta formal y claro para archivo oficial. No incluyas fecha ni título principal, de eso se encarga el sistema.
        """
